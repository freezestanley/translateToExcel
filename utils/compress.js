...
async function CompressImg(link, path, length) {
	try {
    ...
    mkdir(dpath)
    const msg = `${Figures.tick} Compressed [${Chalk.yellowBright(link)}] completed: Old Size ${oldSize}, New Size ${newSize}, Optimization Ratio ${ratio} progress ${length}`;
    spinner.succeed(msg)
...
async function CompressImgs(arr, path, num = 0) {
  let ele_result = []
  function getEle(ar) {
    return new Promise((resolve, reject) => {
      async function single_task(ar) {
        if (num) {
          ...
          task.map(async (ele) => {
            let result = await CompressImg(ele, path, arr.length)
            ele_result.push(result)
          })
        } else {
          let task_link = ar.shift()
          let result = await CompressImg(task_link, path, arr.length)
          ele_result.push(result)
        }
        if (ar.length <= 0) {
          resolve(ele_result)
